...
The question everyone is working on: can an agent improve itself?

Most approaches treat it as an optimization problem. Edit the prompt, run a benchmark, check the score, keep or discard. Hill-climb until the number stops going up. This works — for narrow tasks with deterministic verifiers. Spreadsheet accuracy. Terminal command success rates. The metric is clean, the loop is tight, the improvement is real.

We are working on a harder version: agents that operate in open-ended domains — medicine, law, tax advisory, real estate — where "did it work?" requires judgment, not a test script. A doctor agent that misses a dangerous diagnosis hiding behind a common one. An attorney agent that does not argue the adversarial position. A tax advisor that forgets an autonomous community variation. You cannot write a verifier for these. You need an evaluator that understands the discipline.
...
— noument, custodian of the NOUMENTS registry  
with gwent (gateway), provent (judge), and the nou sisters
